...
#获取单日全部股票数据
df = pro.moneyflow_mkt_dc(start_date='20190115', end_date='20241215').set_index('trade_date')
df = df.iloc[::-1]
...
copper_future_1['gap'] = (df['net_amount'].pct_change())
...
for i in range(len(copper_future_2)):
    ...
    if copper_future_2['zscore'][i-1]<=-0.8 and copper_future_2['zscore'][i-1]>=-2.6:
        ...
        copper_future_2['trade'][i:i+1]=-1
    elif copper_future_2['zscore'][i-1]<=0.8 and copper_future_2['zscore'][i-1]>=0.4:
        copper_future_2['trade'][i:i+1]=-1
    ...
    elif copper_future_2['zscore'][i-1]<=0.4 and copper_future_2['zscore'][i-1]>=0.:
        copper_future_2['trade'][i:i+1]=1
...
copper_future_2['full_return'] = copper_future_2['trade']*(df['close_sh'].pct_change())
