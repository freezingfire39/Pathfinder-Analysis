...
df = pro.moneyflow_mkt_dc(start_date='20190115', end_date='20250315').set_index('trade_date')
df = df.iloc[::-1]
df.index = pd.to_datetime(df.index)

print (df.head(20))

df_2 = pro.index_daily(ts_code='399001.SZ', start_date='20190115', end_date='20250315').set_index('trade_date')
df_2 = df_2.iloc[::-1]
...
df['gap'] = (df['buy_elg_amount_rate'].pct_change()-df['buy_lg_amount_rate'].pct_change())
...
for i in range(len(df)):
    ...
    if df['zscore'][i-2]<=-0.5 and df['zscore'][i-2]>=-1.8:
        df['trade'][i:i+1]=-1
    

    elif df['zscore'][i-2]>=0.5 and df['zscore'][i-2]<=1.8:
        df['trade'][i:i+1]=1
