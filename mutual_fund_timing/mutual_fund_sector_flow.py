...
#获取单日全部股票数据
df = pro.moneyflow_ind_ths(start_date='20190115', end_date='20241115').set_index('trade_date')
df = df.iloc[::-1]
...
copper_future_1 = pro.fut_daily(ts_code='IH.CFX', start_date='20190115', end_date='20241115').set_index('trade_date')
copper_future_2 = pro.fut_daily(ts_code='IF.CFX', start_date='20190115', end_date='20241115').set_index('trade_date')
...
copper_future_1['return'] = copper_future_1['open'].pct_change()
copper_future_2['return'] = copper_future_2['open'].pct_change()
...
copper_future_1['gap'] = (df['net_buy_amount']-df['net_sell_amount']).pct_change()
...
for i in range(len(copper_future_2)):
    ...
    if copper_future_2['zscore'][i-1]<=-0.8 and copper_future_2['zscore'][i-1]>=-2.6:
        copper_future_2['trade'][i:i+1]=1
    

    elif copper_future_2['zscore'][i-1]>=0.8 and copper_future_2['zscore'][i-1]<=2.6:
        copper_future_2['trade'][i:i+1]=-1
    elif copper_future_2['zscore'][i-1]<=0.8 and copper_future_2['zscore'][i-1]>=0.3:
        copper_future_2['trade'][i:i+1]=1
