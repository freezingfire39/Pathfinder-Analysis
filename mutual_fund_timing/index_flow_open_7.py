...
for i in range(len(df['gap'])):
    ...
    data_bp8 = sosfiltfilt(sos, df_temp)
    data_bp2 = butter_bandpass_filter(df_temp,500,2000,20000,2)


    sig_fft = fftpack.fft(df_temp.values)
    ...
    sample_frequency = fftpack.fftfreq(df_temp.size, d=time_step)

#x = pd.DataFrame(data=amplitude, columns=['amplitude'])
#x['frequency'] = pd.Series(sample_frequency)
#x['angle'] = pd.Series(angle)

    df['gap_3'][i] = amplitude[-1]
...
for i in range(len(df)):
    ...
    if df['zscore'][i-2]<=-0.6 and df['zscore'][i-2]>=-1.6:
        df['trade'][i:i+1]=-1
    

    elif df['zscore'][i-2]>=0.6 and df['zscore'][i-2]<=1.6:
        df['trade'][i:i+1]=1

    elif df['zscore'][i-2]<=0.6 and df['zscore'][i-2]>=0.:
        df['trade'][i:i+1]=-1
    ...
    if df['zscore'][i-2]>=1.6:
        df['trade'][i:i+1]=-1
    

    elif df['zscore'][i-2]<=-1.6:
        df['trade'][i:i+1]=1
