...
df_6 = pro.daily(ts_code=df['symbol'].iloc[0], start_date='20180101', end_date='20241229')
df_6 = df_6.iloc[::-1]
df_6.set_index('trade_date',inplace=True)
df_6.index = pd.to_datetime(df_6.index)
df_trade['return'] = df_6['close'].pct_change()
...
for symbol in df['symbol']:
    ...
    df_temp_2 = pro.daily(ts_code=symbol, start_date='20180101', end_date='20241216')
    df_temp_2 = df_temp_2.iloc[::-1]
    df_temp_2.set_index('trade_date',inplace=True)
    df_temp_2.index = pd.to_datetime(df_temp_2.index)
    print (df_temp_2)
    df_temp_2.fillna(0,inplace=True)

    df_2['net_mf_amount']+=df_temp['net_mf_amount']
    df_trade['return']+=df_temp_2['close'].pct_change()
...
df_trade['return'] = df_trade['return']/20
...
for i in range(len(df_trade)):
    ...
    if df_trade['zscore_3'][i-1]<=-0.2 and df_trade['zscore_3'][i-1]>=-0.6:
        df_trade['trade'][i:i+1]=1

    elif df_trade['zscore_3'][i-1]>=0.2 and df_trade['zscore_3'][i-1]<=0.6:
        df_trade['trade'][i:i+1]=-1
