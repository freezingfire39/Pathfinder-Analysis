...
df_2 = pro.income(ts_code=df['symbol'].iloc[0], start_date='20180801', end_date='20241208', fields='total_profit,total_revenue,f_ann_date') ##end date today's date, start_date 5 month minus
...
for symbol in df['symbol']:
    ...
    df_temp = pro.income(ts_code=symbol, start_date='20180801', end_date='20241208', fields='total_profit,total_revenue,f_ann_date') ##end date today's date, start_date 5 month minus
...
df_trade = df_trade.resample("Q").last()
...
df_trade['gap'] = df_2['total_revenue']/df_2['total_profit']
...
for i in range(len(df_trade)):
    ...
    if df_trade['zscore_3'][i-1]<=-0.3 and df_trade['zscore_3'][i-1]>=-1.3:
        df_trade['trade'][i:i+1]=1

    elif df_trade['zscore_3'][i-1]>=0.3 and df_trade['zscore_3'][i-1]<=1.3:
        df_trade['trade'][i:i+1]=-1
