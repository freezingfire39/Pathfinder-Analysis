...
#获取单日全部股票数据
df = pro.moneyflow(ts_code='000001.SZ', start_date='20190115', end_date='20241115').set_index('trade_date')
df = df.iloc[::-1]
...
copper_future_1 = pro.daily(ts_code='600000.SH', start_date='20190115', end_date='20241115').set_index('trade_date')
copper_future_2 = pro.daily(ts_code='000001.SZ', start_date='20190115', end_date='20241115').set_index('trade_date')
...
copper_future_1['gap'] = (df['buy_lg_amount']+df['buy_sm_amount']+df['buy_md_amount']).pct_change()-(df['sell_lg_amount']+df['sell_md_amount']+df['sell_sm_amount']).pct_change()
...
for i in range(len(copper_future_2)):
    ...
    if copper_future_2['zscore'][i-1]<=-0. and copper_future_2['zscore'][i-1]>=-1.6:
        copper_future_2['trade'][i:i+1]=1
    

    elif copper_future_2['zscore'][i-1]>=0. and copper_future_2['zscore'][i-1]<=1.6:
        copper_future_2['trade'][i:i+1]=-1
