...
copper_future_3['gap'] = copper_future_3['交通运输、仓储和邮政业']-copper_future_3['mean']

copper_future_3['zscore'] = (copper_future_3['gap']  - copper_future_3['gap'].rolling(5).mean())/copper_future_3['gap'].rolling(5).std(ddof=0)
copper_future_3['trade'] = 0
copper_future_3['trade'] = copper_future_3['trade'].astype('float64')

for i in range(len(copper_future_3)):
    if i<1:
        continue

    if copper_future_3['zscore'][i-1]>=0.2:
        copper_future_3['trade'][i:i+1]=1


print (copper_future_3)
copper_future_3['full_return'] = copper_future_3['trade']*copper_future_3['房地产业']
import pyfolio_master as pf
#df_trade = pd.to_numeric(df_trade, errors='coerce')

pf.create_full_tear_sheet(copper_future_3['full_return'])
