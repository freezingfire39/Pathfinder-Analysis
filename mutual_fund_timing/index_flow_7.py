...
df['gap'] = (df['buy_lg_amount_rate']-df['buy_md_amount_rate'])
...
for i in range(len(df)):
    ...
    if df['zscore'][i-1]<=-0.7 and df['zscore'][i-1]>=-2.6:
        df['trade'][i:i+1]=-1
    

    elif df['zscore'][i-1]>=0.7 and df['zscore'][i-1]<=2.6:
        df['trade'][i:i+1]=1
    elif df['zscore'][i-1]<=0.7 and df['zscore'][i-1]>=0.2:
        df['trade'][i:i+1]=-1
