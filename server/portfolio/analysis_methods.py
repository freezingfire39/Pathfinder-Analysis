...
class PortfolioAnalysis:
    def __init__(self, rank_file_path, input_file_path):
        PERIODS = OrderedDict()
        # Dotcom bubble
    ...
    def return_analysis(self, returns ):
        ##need to answer three questions 1. is the return high? 2. Where is it coming from? 3. Is it consistent?
        df_benchmark = pd.read_csv(self.rank_file_path+'return_benchmark.csv').set_index('Unnamed: 0')
        df_benchmark_2 = pd.read_csv(self.rank_file_path+'excess_sharpe_benchmark.csv').set_index('Unnamed: 0')
        comment_csv = pd.read_csv(self.input_file_path+'comments.csv').set_index('date')
        if returns['return'][-1]>df_benchmark['value'].quantile(0.6):
            if returns['excess_SR'][-1]>df_benchmark_2['value'].quantile(0.6):
                ...
                comment_csv.to_csv(self.input_file_path+'comments.csv')
        ...
        df_return = returns['return'].resample('3M').sum()
    ...
    def rolling_sharpe(self, returns, rank_file_path,input_file_path,security_code,risk_free_rate=0.0, window=250):
    ...
    def get_top_drawdowns(self, returns, top=10):
        """
    ...
    def gen_drawdown_table(self, returns, rank_file_path,security_code,input_file_path,top=10):
        """
    ...
    def max_drawdown_analysis(self, returns, rank_file_path,security_code,input_file_path):
        returns = self.gen_drawdown_table(returns, rank_file_path,security_code,input_file_path)
    ...
    def extract_interesting_date_ranges(self, returns, periods=None):
        """
    ...
    def event_analysis(self, returns, benchmark_rets=None,
        ...
        rets_interesting = self.extract_interesting_date_ranges(
            returns, periods)

        if not rets_interesting:
            print('Passed returns do not overlap with any'
        ...
        if benchmark_rets is not None:
            returns = self.clip_returns_to_benchmark(returns, benchmark_rets)
    ...
    def alpha_beta_analysis(self,returns, comp, security_code,rank_file_path,input_file_path, window=250):
    ...
    def market_capture_ratio(self, returns, returns_daily, security_code, rank_file_path,input_file_path,rolling_window=250):
        """
    ...
    def corr_analysis(self, returns, comp, security_code, rank_file_path, rank_file_path_2,input_file_path):
        security_code = str(security_code)

        comp[security_code] = (1+returns['return']).cumprod()
        ...
        #     returns['index_peers']=""
        #     returns['index_peers'].iloc[-1] = corr_df_2.columns[np.argsort(-1*corr_df_2.tail(1).values,axis=1)[:, :3].tolist()[0]]
        #
        #
        # else:
        #     returns['industry_peers']=""
        #     returns['industry_peers'].iloc[-1] = corr_df_2.columns[np.argsort(-1*corr_df_2.tail(1).values,axis=1)[:, :3].tolist()[0]]

        comment_csv = pd.read_csv(input_file_path + '/'+'comments.csv').set_index('date')
        returns['positive_comp'] = ""
        returns['negative_comp'] = ""
    ...
    def rolling_volatility(self, returns, comp, rank_file_path,security_code,input_file_path,rolling_vol_window=250):
        """
