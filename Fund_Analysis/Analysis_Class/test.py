...
def main(symbol_file_path,symbol,search_file_path):
    ...
    df_target['return'] = df['return'].apply(lambda x: "{:.2%}".format(x))
    df_target['net_return'] = df['net_return'].apply(lambda x: "{:.2%}".format(x))
    df_target['comp_1'] = df['net_return'].apply(lambda x: "{:.2%}".format(x))
    df_target['vol'] = df['net_return'].apply(lambda x: "{:.2%}".format(x))
