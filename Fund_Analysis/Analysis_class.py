...
def gen_drawdown_table(returns, top=10):
    ...
    print (df_drawdowns['Duration'].mean())
    if df_drawdowns['Duration'].mean()>100:
...
def corr_analysis(returns,comp, security_code):
    ...
    elif corr_df[comp_2_name][-1] < -0.9:
        ##save comp_2_name
        if comp_1_name=="510050.SS":
    ...
    return comp_1_name,comp_2_name
...
def rolling_volatility(returns, comp, rolling_vol_window=90):
    ...
    return returns['return'].rolling(rolling_vol_window).std() \
