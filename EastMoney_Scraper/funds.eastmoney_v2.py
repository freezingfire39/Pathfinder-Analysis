...
# records = records[:100]  # Limiting the number of records to 5 for testing purposes
...
class scrapySpider(scrapy.Spider):
   ...
   custom_settings = {
      # "CONCURRENT_REQUESTS": 1,  # Set the number of concurrent requests to 1
      ...
      "CONCURRENT_REQUESTS": 100,  # Set the number of concurrent requests to 100
   }

   # Initialize an empty set to keep track of records for which save_data has been called
   processed_records = set()
   
   # Initialize a set to store stop call for each record
   stop_calls = set()
   ...
   def start_requests(self):
      try:
         ...
         for record in records:
            ...
            # Check if the user has chosen to scrape Asset, if yes, proceed with scraping
            if scrape_Asset == "y":
               # Define URL for Asset data API
               Asset_url = "https://fundf10.eastmoney.com/FundArchivesDatas.aspx"

               # Define query parameters for Asset data API
               Asset_url_querystring = {
                  "type":"gmbd",
                  "mode":"0",
                  "code":f"{record}"
               }

               # Define headers for the HTTP request to Asset data API
               headers = {
                  "Accept": "*/*",
                  "Accept-Language": "en-US,en;q=0.9,so;q=0.8",
                  "Connection": "keep-alive",
                  "Referer": "https://fundf10.eastmoney.com/gmbd_510570.html",
               ...
               # Construct full URL for the Asset data API request
               full_Asset_url = f"{Asset_url}?{'&'.join(f'{k}={v}' for k, v in Asset_url_querystring.items())}"
   ...
   def parse_fund_TotalCount(self, response):
      ...
      try:
         ...
         for current_page in range(1, total_pages + 1):
            ...
            self.logger.info(f"Fetching fund data for record: {record}, page: {current_page}")
   ...
   def parse_fund(self, response):
      ...
      if first_table:
         ...
         if fund_data:
            # Save fund data
            self.save_data(record, fund_data, csv_name="Fund_1", first_time=first_time, date=date)
      ...
      if second_table:
         ...
         if fund_data:
            # Save fund data
            self.save_data(record, fund_data, csv_name="Fund_2", first_time=first_time, date=date)
   ...
   # Define a method to parse Asset data for a specific record and save it to a CSV file
   def parse_Asset(self, response):
      # Extract the record from the response metadata
      record = response.meta['record']

      # Initialize variables to store Asset data
      日期 = ""
      期间申购 = ""
      期间赎回 = ""
      期末总份额 = ""
      ...
      table = response.text
      table = table.split("var gmbd_apidata={ content:\"")[1]
      table = table.split("\",summary:")[0]

      # Parse the table HTML with Selector
      ...
      for row in rows:
         ...
         日期 = row.xpath('.//td[1]/text()').extract_first()
         期间申购 = row.xpath('.//td[2]/text()').extract_first()
         期间赎回 = row.xpath('.//td[3]/text()').extract_first()
         期末总份额 = row.xpath('.//td[4]/text()').extract_first()
         期末净资产 = row.xpath('.//td[5]/text()').extract_first()
         净资产变动率 = row.xpath('.//td[6]/text()').extract_first()
         ...
         # print("期间申购（亿份）:", 期间申购)
         # print("期间赎回（亿份）:", 期间赎回)
         # print("期末总份额（亿份）:", 期末总份额)
         # print("期末净资产（亿元）:", 期末净资产)
         # print("净资产变动率:", 净资产变动率)
         # print("--------------------------------------------")

         # Create a dictionary to store Asset data for a single row
         Asset_data_row = {
            "日期": 日期,
            "期间申购（亿份）": 期间申购,
            "期间赎回（亿份）": 期间赎回,
         ...
         # Append the Asset data row to the Asset_data list
         Asset_data.append(Asset_data_row)

      # Log the parsing of Asset for a specific record
      self.logger.info(f"Parsing Asset for record: {response.meta['record']}")

      if Asset_data:
         # Save Asset data
         self.save_data(record, Asset_data, csv_name="Asset")
   ...
   def save_data(self, record, data, csv_name, first_time=False, date=""):
      ...
      # Save data to Asset.csv
      if csv_name == "Asset":
         Asset_csv = open(f'{folder_name}/{csv_name}.csv', 'w', newline='', encoding="utf-8-sig")
         Asset_field_names = ['日期', '期间申购（亿份）', '期间赎回（亿份）', '期末总份额（亿份）', '期末净资产（亿元）', '净资产变动率']
         Asset_writer = csv.DictWriter(Asset_csv, fieldnames=Asset_field_names)
         
         # Write header row to Asset.csv
         Asset_writer.writeheader()

         # Write data to Asset.csv
         for data_row in data:
            Asset_writer.writerow(data_row)
         
         # Close Asset.csv file
         Asset_csv.close()
